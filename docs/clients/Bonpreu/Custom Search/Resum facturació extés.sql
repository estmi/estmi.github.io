...
-- END MAXIMETRES
            (SELECT 
                CASE
                    WHEN SUM(linia.quantity) <> 0 THEN SUM(linia.price_subtotal)/SUM(linia.quantity)
                    ELSE 0
                END 
            FROM giscedata_facturacio_factura_linia flinia
            JOIN account_invoice_line linia ON linia.id=flinia.invoice_line_id
        ...
        FROM
                ...
                INNER JOIN res_partner part_pol ON part_pol.id=pol.titular
        ...
        AND fact.data_inici BETWEEN mod.data_inici AND mod.data_final  AND  part_pol.vat LIKE '%$nif_titular'