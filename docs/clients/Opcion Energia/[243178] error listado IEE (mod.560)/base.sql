...
-- DESCUENTOS
		    ...
		    COALESCE(it_21.name, '') AS IVA2,
   		    COALESCE(it_21.base,0)  * (CASE WHEN inv.type like '%%_refund' THEN -1 ELSE 1 END) AS "Base IVA2",
   		    COALESCE(it_21.amount,0)  * (CASE WHEN inv.type like '%%_refund' THEN -1 ELSE 1 END) AS "total iva2",
      ...
      COALESCE(it_0.amount,0)  * (CASE WHEN inv.type like '%%_refund' THEN -1 ELSE 1 END) AS "total iva3",
        ...
        FROM
                ...
                LEFT JOIN account_invoice_tax it_iese ON (it_iese.invoice_id=inv.id AND it_iese.name LIKE '%%electricidad%%')
				...
				LEFT JOIN account_invoice_tax it_21 ON (it_21.invoice_id=inv.id AND (it_21.name LIKE 'IVA 21%%' OR it_21.name LIKE '21%%%% IVA%%'))
