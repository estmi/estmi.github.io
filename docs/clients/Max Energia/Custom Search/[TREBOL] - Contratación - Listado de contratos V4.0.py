...
SELECT 
    ...
    tarifa.id as tarifa_regulada_id,
...
# {tarifa:{P1_te:{pricelist_id:price}}}
tarifes = {}
# {pricelist:bool}
pricelists = {}

def OmpleTarifaPricelist(cursor,uid,tarifes,tarifa_id,pl_id,llistapreus_o,tarifas_o,periodes_o,ctx,row):
    tarifes[tarifa_id]= {}
    llistapreus_data = llistapreus_o.read(cursor, uid, pl_id, ['compatible_invoicing_modes'])
    periodes = tarifas_o.read(cursor, uid, tarifa_id, ['periodes'])['periodes']
    periodes_data = periodes_o.read(cursor, uid, periodes, ['product_id', 'name', 'tipus'], context=ctx)
    for periode in periodes_data:
        ...
        if periode['tipus'] == 'te':
            if llistapreus_data['compatible_invoicing_modes'][0] == 1:
                try:
                    preu = llistapreus_o.price_get(cursor, uid, pl_id, product_id, 1, None, ctx)[pl_id]
                except:
                    preu = 0
            else:
                preu = 0
            key = 'preu_energia_{}'.format(periode['product_id'][1].lower())
            row[key] = preu
            if key in tarifes[tarifa_id]:
                tarifes[tarifa_id][key][pl_id] = preu
            else:
                tarifes[tarifa_id][key] = {pl_id:preu}
        else:
            try:
                ...
                preu = 0
            key = 'preus_potencia_{}'.format(periode['product_id'][1].lower())
            row[key] = preu
            if key in tarifes[tarifa_id]:
                tarifes[tarifa_id][key][pl_id] = preu
            else:
                tarifes[tarifa_id][key] = {pl_id:preu}
def Preu(row,key, pl_id, tarifa):
    if key in tarifa.keys():
        row[key] = tarifa[key][pl_id]
    else:
        row[key] = '0'
for row in tqdm(res):
    pl_id = llistapreus_o.search(cursor, uid, [('name','ilike', row['nombre_tarifa'])], context=ctx)[0]
    if pl_id:
        tarifa_id = row['tarifa_regulada_id']
        if tarifa_id in tarifes:
            if pl_id in tarifes[tarifa_id]['preus_potencia_p1']:
                Preu(row,'preus_energia_p1', pl_id, tarifes[tarifa_id])
                Preu(row,'preus_energia_p2', pl_id, tarifes[tarifa_id])
                Preu(row,'preus_energia_p3', pl_id, tarifes[tarifa_id])
                Preu(row,'preus_energia_p4', pl_id, tarifes[tarifa_id])
                Preu(row,'preus_energia_p5', pl_id, tarifes[tarifa_id])
                Preu(row,'preus_energia_p6', pl_id, tarifes[tarifa_id])
                Preu(row,'preus_potencia_p1', pl_id, tarifes[tarifa_id])
                Preu(row,'preus_potencia_p2', pl_id, tarifes[tarifa_id])
                Preu(row,'preus_potencia_p3', pl_id, tarifes[tarifa_id])
                Preu(row,'preus_potencia_p4', pl_id, tarifes[tarifa_id])
                Preu(row,'preus_potencia_p5', pl_id, tarifes[tarifa_id])
                Preu(row,'preus_potencia_p6', pl_id, tarifes[tarifa_id])
            else:
                OmpleTarifaPricelist(cursor,uid,tarifes,tarifa_id,pl_id,llistapreus_o,tarifas_o,periodes_o,ctx,row)
        else:    
            OmpleTarifaPricelist(cursor,uid,tarifes,tarifa_id,pl_id,llistapreus_o,tarifas_o,periodes_o,ctx,row)