SELECT
            ...
            (select CASE WHEN inv.type in ('out_refund', 'in_refund') THEN  SUM(ait.base)  * -1
                ELSE SUM(ait.base) END from account_invoice_tax ait where ait.invoice_id = inv.id and ait.name ilike '%IVA%')
            AS "Base imponible (IVA)",
            (select CASE WHEN inv.type in ('out_refund', 'in_refund') THEN  SUM(ait.amount)  * -1
                ELSE  SUM(ait.amount) END from account_invoice_tax ait where ait.invoice_id = inv.id and ait.name ilike '%IVA%')
            AS "Impuestos (IVA)",
            CASE
