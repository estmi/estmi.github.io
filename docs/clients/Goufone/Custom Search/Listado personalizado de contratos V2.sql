...
(
    COALESCE(
        COALESCE(
            (SELECT c102.data_activacio FROM giscedata_switching_c1_02 c102 JOIN giscedata_switching_step_header c102h ON c102h.id=c102.header_id WHERE c102h.sw_id = sw_alta.id),
            (SELECT c202.data_activacio FROM giscedata_switching_c2_02 c202 JOIN giscedata_switching_step_header c202h ON c202h.id=c202.header_id WHERE c202h.sw_id = sw_alta.id)
         ),
        (SELECT a302.data_activacio FROM giscedata_switching_a3_02 a302 JOIN giscedata_switching_step_header a302h ON a302h.id=a302.header_id WHERE a302h.sw_id = sw_alta.id)
     )
...
-- ------
    ...
    (SELECT STRING_AGG('[' || prod.default_code || '] ' || prod_template.name, ' | ')
    FROM giscedata_facturacio_services ser 
    LEFT JOIN product_product prod ON prod.id = ser.producte 
    LEFT JOIN product_template prod_template ON prod.product_tmpl_id = prod_template.id 
    WHERE ser.polissa_id = pol.id AND ser.data_inici < CURRENT_DATE AND (ser.data_fi > CURRENT_DATE OR ser.data_fi IS NULL))
...
WHERE
    pol.state IN ('esborrany', 'activa', 'baixa', 'baixa2', 'modcontractual')
    AND pol.name = '06129'
