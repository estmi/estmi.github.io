...
(
    SELECT potencia FROM giscedata_polissa_potencia_contractada_periode pot JOIN giscedata_polissa_tarifa_periodes per ON per.id=pot.periode_id JOIN product_product prod ON prod.id=per.product_id JOIN product_template tmp ON tmp.id=prod.product_tmpl_id WHERE pot.polissa_id = pol.id AND tmp.name = 'P1' 
) as potencia_contratada_p1,
(
    SELECT potencia FROM giscedata_polissa_potencia_contractada_periode pot JOIN giscedata_polissa_tarifa_periodes per ON per.id=pot.periode_id JOIN product_product prod ON prod.id=per.product_id JOIN product_template tmp ON tmp.id=prod.product_tmpl_id WHERE pot.polissa_id = pol.id AND tmp.name = 'P2' 
) as potencia_contratada_p2,
(
    SELECT potencia FROM giscedata_polissa_potencia_contractada_periode pot JOIN giscedata_polissa_tarifa_periodes per ON per.id=pot.periode_id JOIN product_product prod ON prod.id=per.product_id JOIN product_template tmp ON tmp.id=prod.product_tmpl_id WHERE pot.polissa_id = pol.id AND tmp.name = 'P3' 
) as potencia_contratada_p3,
(
    SELECT potencia FROM giscedata_polissa_potencia_contractada_periode pot JOIN giscedata_polissa_tarifa_periodes per ON per.id=pot.periode_id JOIN product_product prod ON prod.id=per.product_id JOIN product_template tmp ON tmp.id=prod.product_tmpl_id WHERE pot.polissa_id = pol.id AND tmp.name = 'P4' 
) as potencia_contratada_p4,
(
    SELECT potencia FROM giscedata_polissa_potencia_contractada_periode pot JOIN giscedata_polissa_tarifa_periodes per ON per.id=pot.periode_id JOIN product_product prod ON prod.id=per.product_id JOIN product_template tmp ON tmp.id=prod.product_tmpl_id WHERE pot.polissa_id = pol.id AND tmp.name = 'P5' 
) as potencia_contratada_p5,
(
    SELECT potencia FROM giscedata_polissa_potencia_contractada_periode pot JOIN giscedata_polissa_tarifa_periodes per ON per.id=pot.periode_id JOIN product_product prod ON prod.id=per.product_id JOIN product_template tmp ON tmp.id=prod.product_tmpl_id WHERE pot.polissa_id = pol.id AND tmp.name = 'P6' 
...
(
    COALESCE(
        COALESCE(
            (SELECT max(c102.data_activacio) FROM giscedata_switching_c1_02 c102 JOIN giscedata_switching_step_header c102h ON c102h.id=c102.header_id WHERE c102h.sw_id = sw_alta.id),
            (SELECT max(c202.data_activacio) FROM giscedata_switching_c2_02 c202 JOIN giscedata_switching_step_header c202h ON c202h.id=c202.header_id WHERE c202h.sw_id = sw_alta.id)
         ),
        (SELECT max(a302.data_activacio) FROM giscedata_switching_a3_02 a302 JOIN giscedata_switching_step_header a302h ON a302h.id=a302.header_id WHERE a302h.sw_id = sw_alta.id)
     )
...
(
    COALESCE(
            (SELECT max(c111.data_activacio) FROM giscedata_switching_c1_11 c111 JOIN giscedata_switching_step_header c111h ON c111h.id=c111.header_id WHERE c111h.sw_id = sw_baixa.id),
            (SELECT max(c211.data_activacio) FROM giscedata_switching_c2_11 c211 JOIN giscedata_switching_step_header c211h ON c211h.id=c211.header_id WHERE c211h.sw_id = sw_baixa.id)
     )
...
for row in tqdm(res):
    pl_id = llistapreus_o.search(cursor, uid, [('name','ilike', row['nombre_tarifa'])], context=context)
    if pl_id:
        tarifa_id = tarifas_o.search(cursor, uid, [('name', '=', row['tarifa_regulada'])])[0]
        llistapreus_data = llistapreus_o.read(cursor, uid, pl_id[0], ['compatible_invoicing_modes'])
        periodes = tarifas_o.read(cursor, uid, tarifa_id, ['periodes'])['periodes']
        for periode in periodes:
            periode_data = periodes_o.read(cursor, uid, periode, ['product_id', 'name', 'tipus'], context=context)
            product_id = periode_data['product_id'][0]
            ctx = {'date': datetime.today().strftime('%Y-%m-%d')}
            if periode_data['tipus'] == 'te':
                if llistapreus_data['compatible_invoicing_modes'][0] == 1:
                    try:
                        preu = llistapreus_o.price_get(cursor, uid, pl_id, product_id, 1, None, ctx)[pl_id[0]]
                    except:
                        preu = 0
                else:
                    preu = 0
                key = 'preu_energia_{}'.format(periode_data['product_id'][1].lower())
                row[key] = preu
            else:
                try:
                    preu = llistapreus_o.price_get(cursor, uid, pl_id, product_id, 1, None, ctx)[pl_id[0]]
                    ...
                    preu = 0
                key = 'preus_potencia_{}'.format(periode_data['product_id'][1].lower())
                row[key] = preu